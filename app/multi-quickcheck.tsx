...
function contextKey(row: CheckRow) {
  return [row.scanner_profile_id ?? '(none)', row.scanner_model_id ?? '(none)', Number(row.scanner_strength_t), (row.scan_region ?? '').trim().toLowerCase()].join('|');
}
...
export default function MultiQuickCheckScreen() {
  ...
  return (
    <ScrollView contentInsetAdjustmentBehavior="automatic" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={load} />} contentContainerStyle={{ padding: 16, paddingBottom: 60, gap: 16 }}>
      <View style={{ gap: 5 }}>
        <Text selectable style={{ color: palette.text, fontSize: 28, fontWeight: '900', letterSpacing: -0.8 }}>Multi-implant assessment</Text>
        <Text selectable style={{ color: palette.muted, fontSize: 14, lineHeight: 20 }}>Run one exact QuickCheck for every implanted device, then combine the matching checks below. A cleared implant can never override an unresolved or conflicting implant.</Text>
      </View>
      ...
      <View style={{ backgroundColor: palette.conditionalSoft, borderRadius: radii.md, padding: 14, gap: 5 }}>
        ...
        <Text selectable style={{ color: palette.text, fontSize: 12, lineHeight: 17 }}>Do not enter or record patient name, date of birth, MRN, accession number, or other identifying information in this workflow.</Text>
      </View>

      <Link href="/quickcheck" asChild><Pressable style={{ minHeight: 50, borderRadius: radii.md, backgroundColor: palette.brand, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: palette.white, fontSize: 15, fontWeight: '900' }}>Run another implant QuickCheck</Text></Pressable></Link>

      {selectedRows.length > 0 ? <View style={{ backgroundColor: palette.brandSoft, borderRadius: radii.lg, borderWidth: 1, borderColor: palette.brand, padding: 15, gap: 8 }}>
        ...
        <Text selectable style={{ color: palette.muted, fontSize: 12, lineHeight: 17 }}>{selectedRows[0].scanner_strength_t}T · {selectedRows[0].scan_region || 'scan region not recorded'} · same saved scanner/exam context required</Text>
        <Pressable onPress={() => { setSelectedIds([]); setCombined(null); setMessage(''); }} style={{ alignSelf: 'flex-start', paddingVertical: 6 }}><Text style={{ color: palette.brand, fontWeight: '800' }}>Clear selection</Text></Pressable>
      </View> : null}
      ...
      <View style={{ gap: 8 }}><Text selectable style={{ color: palette.text, fontSize: 18, fontWeight: '900' }}>Recent exact QuickChecks</Text><Text selectable style={{ color: palette.muted, fontSize: 12, lineHeight: 17 }}>Select at least two checks performed for the same saved scanner and scan region.</Text></View>

      {!rows.length && !refreshing ? <View style={{ backgroundColor: palette.surface, borderRadius: radii.lg, padding: 22, alignItems: 'center', gap: 10 }}>
      ...
      {rows.map((row) => {
        ...
        return <Pressable key={row.id} onPress={() => toggleRow(row)} style={{ backgroundColor: selected ? palette.brandSoft : palette.surface, borderRadius: radii.lg, borderWidth: selected ? 2 : 1, borderColor: selected ? palette.brand : palette.line, padding: 15, gap: 8 }}>
          <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
            <View style={{ flex: 1, gap: 3 }}><Text selectable style={{ color: palette.text, fontSize: 15, fontWeight: '900' }}>{deviceLabel(row)}</Text><Text selectable style={{ color: colors.foreground, fontSize: 12, fontWeight: '900' }}>{display}</Text><Text selectable style={{ color: palette.muted, fontSize: 12 }}>{row.scanner_strength_t}T · {row.scan_region || 'region not recorded'} · {new Date(row.created_at).toLocaleString()}</Text></View>
            <View style={{ width: 28, height: 28, borderRadius: 14, borderWidth: 2, borderColor: selected ? palette.brand : palette.line, alignItems: 'center', justifyContent: 'center' }}>{selected ? <View style={{ width: 14, height: 14, borderRadius: 7, backgroundColor: palette.brand }} /> : null}</View>
          </View>
          <Text selectable style={{ color: palette.muted, fontSize: 11, lineHeight: 16 }}>{row.result?.safe_to_scan === true && row.result?.conditions_met === true ? 'Exact result recorded with displayed conditions met.' : row.result?.hard_conflict === true ? 'Hard conflict recorded — this will control the combined patient result.' : 'This implant is still unresolved or has unconfirmed conditions.'}</Text>
        </Pressable>;
      })}

      <Pressable disabled={selectedIds.length < 2 || busy} onPress={combine} style={{ minHeight: 56, opacity: selectedIds.length < 2 || busy ? 0.45 : 1, borderRadius: radii.md, backgroundColor: palette.brand, alignItems: 'center', justifyContent: 'center' }}>{busy ? <ActivityIndicator color={palette.white} /> : <Text style={{ color: palette.white, fontSize: 16, fontWeight: '900' }}>Combine Selected Implants</Text>}</Pressable>

      {combined ? <View style={{ gap: 10 }}>
      ...
      <Text selectable style={{ color: palette.muted, textAlign: 'center', fontSize: 12, lineHeight: 17 }}>Combined status is fail-closed. Every implanted device must be represented by an exact QuickCheck performed for the same scanner and exam. Current manufacturer labeling and institutional MRI safety review remain authoritative.</Text>
    </ScrollView>
  );
}
